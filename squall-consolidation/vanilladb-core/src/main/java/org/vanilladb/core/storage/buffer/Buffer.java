...
//			modifiedBy.add(txNum);
	...
	public void setPageMetadata(PageMetadata newData) {
		internalLock.writeLock().lock();
		try {
			cleanMetaData();
			pageMetadata = newData;
		} finally {
			internalLock.writeLock().unlock();
		}
	}
