...
public class OldTPartCacheMgr {
	...
	public CachedRecord read(int sinkId, RecordKey key, long srcTxNum,
			Transaction tx) {
		// System.out.println("read:" + key + " src:" + srcTxNum + " tx:"
		// + tx.getTransactionNumber());
		SinkCachePool scp;
		synchronized (sinkCachePoolMap) {
			scp = sinkCachePoolMap.get(sinkId);
		...
		if (rec != null) // hit in local sink pool
			return rec;
		...
		synchronized (scp) {
			...
			if (waitRemote)
				try {
					while (waitRemote) {
						// System.out.println("caceh wait " + key + " src:"
						// + srcTxNum + " tx:" + tx.getTransactionNumber()
						// + " sink:" + sinkId);
						scp.wait();
						waitRemote = scp.remoteFlags.contains(rv);
					}
