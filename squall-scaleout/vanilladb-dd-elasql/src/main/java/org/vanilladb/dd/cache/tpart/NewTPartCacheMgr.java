...
public class NewTPartCacheMgr implements CacheMgr {
	private Map<CachedEntryKey, CachedRecord> cachedRecordMap = new ConcurrentHashMap<CachedEntryKey, CachedRecord>();
	private Set<CachedEntryKey> remoteFlags = Collections
			.synchronizedSet(new HashSet<CachedEntryKey>());
	...
	public NewTPartCacheMgr() {
		...
		mySinkId = getPartitionTxnId(VanillaDdDb.serverId());
	...
	public void update(RecordKey key, Map<String, Constant> fldVals,
		...
		if (dests != null) {
			...
			for (int i = 0; i < dests.length; i++) {
				CachedEntryKey k = new CachedEntryKey(key,
						tx.getTransactionNumber(), dests[i]);
				synchronized (prepareAnchor(k)) {
	...
	public void writeBack(RecordKey key, long src, int sinkProcessId,
			Transaction tx) {
	 ...
	 * 
	 */
	public void createCacheRecordFromSink(RecordKey key, int mySinkProcessId,
			Transaction tx, long... dests) {
	...
	public CachedRecord readFromSink(RecordKey key, int mySinkProcessId,
			Transaction tx) {
	...
	public void cacheRemoteRecord(RecordKey key, long src, long dest,
			CachedRecord rec) {
		CachedEntryKey k = new CachedEntryKey(key, src, dest);
