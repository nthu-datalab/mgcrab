...
import netdb.software.benchmark.tpcc.procedure.vanilladddb.DdTestbedLoaderProc;
...
public class TestbedLoaderProc implements CalvinStoredProcedure {
	...
	public void loadTestbed() {
		...
		try {
			generateItems();
			if (DdTestbedLoaderProc.WAREHOUSE_IDS == null)
				for (int i = 1; i <= TpccConstants.NUM_WAREHOUSES; i++)
					generateWarehouseInstance(i);
			else
				for (int i = 0; i < DdTestbedLoaderProc.WAREHOUSE_IDS.length; i++) {
					generateWarehouseInstance(DdTestbedLoaderProc.WAREHOUSE_IDS[i]);
				}
		} catch (Exception e) {
			e.printStackTrace();
			tx.rollback();
