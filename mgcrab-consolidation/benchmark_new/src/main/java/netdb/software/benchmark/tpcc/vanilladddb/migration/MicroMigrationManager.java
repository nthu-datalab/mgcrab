...
//		return partId == this.getDestPartition();
		// For consolidation
		return partId == this.getSourcePartition();
	}
	...
	@Override
	public void onReceiveStartCrabbingReq(Object[] metadata) {
		// Send a store procedure call
		Object[] call = { new StoredProcedureCall(-1, -1, TransactionType.START_CRABBING.ordinal(), (Object[]) null) };
		VanillaDdDb.connectionMgr().sendBroadcastRequest(call);
	}
...
//		}
		int startId = VanillaDdDb.migrationMgr().getSourcePartition() * TpccConstants.YCSB_MAX_RECORD_PER_PART + 1;
