...
public class BTreeDir {
	...
	BTreeDir(String dataFileName, BlockId blk, TableInfo ti, Transaction tx) {
		...
		if (blk.number() == 0) { // Root block
			contents = BTreePage.newBTreePageFromCache(null, -1, blk, BTreeDir.NUM_FLAGS, ti, tx);
		} else {
			contents = new BTreePage(blk, NUM_FLAGS, true, ti, tx);
		}
	}
	...
	public void makeNewRoot(DirEntry e) {
		...
		if (contents.currentBlk().number() != 0) {
			contents.close();
			contents = new BTreePage(new BlockId(ti.fileName(), 0), NUM_FLAGS, true, ti, tx);
		}
		Constant firstval = getKey(contents, 0);
		long level = getLevelFlag(contents);
		// transfer all records to the new block
		long newBlkNum = contents.split(0, new long[] { level }, true);
	...
	public DirEntry insert(DirEntry e) {
		try {
			...
			BlockId childBlk = new BlockId(ti.fileName(), getChildBlockNumber(contents, findChildSlotNumber(e.key())));
	...
	public DirEntry insertEntry(DirEntry e) {
		...
		long newBlkNum = contents.split(splitPos, new long[] { getLevelFlag(contents) }, true);
	...
	private BlockId searchForInsert(Constant searchKey, TableInfo leafTi) {
		...
		try {
			...
			int slotNum = findChildSlotNumber(searchKey);
			long childBlkNum = getChildBlockNumber(contents, slotNum);
			BlockId childBlk;
			...
			while (getLevelFlag(contents) > 0) {
				...
				ccMgr.crabDownDirBlockForModification(childBlk);
				BTreePage child = BTreePage.newBTreePageFromCache(contents, slotNum, childBlk, NUM_FLAGS, ti, tx);
				...
				contents.close();
				contents = child;
				slotNum = findChildSlotNumber(searchKey);
				childBlkNum = getChildBlockNumber(contents, slotNum);
	...
	private int findChildSlotNumber(Constant searchKey) {
