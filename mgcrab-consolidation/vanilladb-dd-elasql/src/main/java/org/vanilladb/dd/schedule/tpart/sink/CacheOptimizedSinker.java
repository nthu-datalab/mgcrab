...
import org.vanilladb.dd.schedule.tpart.Edge;
import org.vanilladb.dd.schedule.tpart.Node;
...
public class CacheOptimizedSinker extends Sinker {
	...
	private List<TPartStoredProcedureTask> createSunkPlan(TGraph graph) {
		...
		for (Node node : graph.getNodes()) {
			...
			plan.setIsLocalTask(taskIsLocal);
			...
			for (Edge e : node.getReadEdges()) {
				...
				if (taskIsLocal) {
					...
					plan.addSinkPushingInfo(e.getResourceKey(),
							node.getPartId(),
							NewTPartCacheMgr.getPartitionTxnId(myId),
							node.getTxNum());
				}
			}
			...
			if (taskIsLocal) {
				for (Edge e : node.getWriteEdges()) {
					...
					if (targetServerId != myId)
						plan.addPushingInfo(e.getResourceKey(), targetServerId,
								txNum, e.getTarget().getTxNum());

					plan.addWritingInfo(e.getResourceKey(), e.getTarget()
							.getTxNum());
				}
			}
					...
					if (taskIsLocal) {
						if (targetServerId == myId) {
							...
							plan.addPushingInfo(k, targetServerId, txNum,
									NewTPartCacheMgr
											.getPartitionTxnId(targetServerId));

						}
						plan.addWritingInfo(e.getResourceKey(),
								NewTPartCacheMgr
										.getPartitionTxnId(targetServerId));
					} else {
						if (targetServerId == myId) {
