package org.vanilladb.dd.storage.log;
 ...
 */
public class DdLogMgr implements Iterable<BasicLogRecord> {
	/**
	...
	static {
		String prop = System.getProperty(DdLogMgr.class.getName() + ".LOG_FILE");
		LOG_FILE = (prop == null ? "vanilladddb.log" : prop.trim());
	}
	...
	public long append(Constant[] rec) {
		...
		try {
			...
			// if(count % 10000 == 0){
			// System.out.println("logMgr count: " + count);
			// }
			...
			if (currentPos + recsize >= BLOCK_SIZE) {
				// System.out.println("append flush");
				flush();
				appendNewBlock();
			}
			for (Constant c : rec)
