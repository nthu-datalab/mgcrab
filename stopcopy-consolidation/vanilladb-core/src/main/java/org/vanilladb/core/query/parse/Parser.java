...
public class Parser {
	...
	static {
		String prop = System.getProperty(Parser.class.getName()
				+ ".DEFAULT_IDX");
		DEFAULT_IDX = (prop == null ? IDX_BTREE : Integer.parseInt(prop.trim()));
	}
	...
	private CreateIndexData createIndex() {
		...
		String fldname = id();
		lex.eatDelim(')');
		return new CreateIndexData(idxname, tblname, fldname, DEFAULT_IDX);
	}
}
