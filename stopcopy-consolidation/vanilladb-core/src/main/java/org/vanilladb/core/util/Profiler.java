...
public class Profiler implements Runnable {
	...
	static {
		String prop = System
				.getProperty(Profiler.class.getName() + ".INTERVAL");
		INTERVAL = (prop == null ? 10 : Integer.parseInt(prop.trim()));
		prop = System.getProperty(Profiler.class.getName() + ".DEPTH");
		DEPTH = (prop == null ? 5 : Integer.parseInt(prop.trim()));
		...
		if (prop != null)
			IGNORE_THREADS = prop.trim().split(",");
		else
			IGNORE_THREADS = new String[] {
					...
					"un.awt.windows.WToolkit.eventLoop",
					"sun.misc.Unsafe.park",
					"dalvik.system.VMStack.getThreadStackTrace",
					"dalvik.system.NativeStart.run" };

		prop = System
				.getProperty(Profiler.class.getName() + ".IGNORE_PACKAGES");
		if (prop != null)

			IGNORE_PACKAGES = prop.trim().split(",");
		else
			IGNORE_PACKAGES = new String[] { "java.", "javax.", "sun.", "net." };
	}
