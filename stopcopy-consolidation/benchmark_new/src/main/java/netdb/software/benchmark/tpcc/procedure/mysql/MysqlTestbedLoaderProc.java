...
public class MysqlTestbedLoaderProc implements StoredProcedure {
	private static Logger logger = Logger
			.getLogger(MysqlTestbedLoaderProc.class.getName());
	...
	private void loadTestbed() {
		...
		try {
			...
			stm = MysqlService.createStatement(conn);
			rs = null;
	...
	private void generateCustomers(int wid, int did) {
		...
		for (int i = 1; i <= TpccConstants.CUSTOMERS_PER_DISTRICT; i++) {
			cid = i;
			if (i > TpccConstants.NUM_DISTINCT_CLAST)
