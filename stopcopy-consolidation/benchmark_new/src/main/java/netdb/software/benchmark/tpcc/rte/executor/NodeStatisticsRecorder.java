...
public class NodeStatisticsRecorder extends Thread {
	...
	private static final long RECORD_INTERVAL = 3000; // in millisecond
	...
	public NodeStatisticsRecorder(int nodeCount, long startTime, long recordInterval) {
		...
		this.totalTime = TestingParameters.WARM_UP_INTERVAL + TestingParameters.BENCHMARK_INTERVAL + recordInterval;
	...
	public void run() {
		...
		while (elapsedTime < totalTime) {
			...
			if (elapsedTime >= (recordCount + 1) * recordInterval) {
				...
				sb.append("- Total: " + totalTxCount + "\n");
				sb.append("- Each Nodes: " + stats[0].txCount);
				...
				sb.append("Latency:\n");
				sb.append("- Total Average (in us): " + avgLatency + "\n");
				sb.append("- Each Nodes: " + avgNodeLatency[0]);
				for (int i = 1; i < nodeCount; i++) {
					sb.append(", ");
