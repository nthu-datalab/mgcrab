...
public class DdTestbedLoaderProc implements DdStoredProcedure {
	private static Logger logger = Logger.getLogger(DdTestbedLoaderProc.class
			.getName());
	public static int[] WAREHOUSE_IDS;
	...
	static {
		String prop = System.getProperty(DdTestbedLoaderProc.class.getName()
				+ ".WAREHOUSE_IDS");
		if (prop != null && !prop.isEmpty()) {
			String[] widInfo = prop.split(",")[VanillaDdDb.serverId()]
					.split(":");
			WAREHOUSE_IDS = new int[widInfo.length];
			for (int i = 0; i < widInfo.length; i++)
				WAREHOUSE_IDS[i] = Integer.parseInt(widInfo[i].trim());
		}
	}

	public DdTestbedLoaderProc(long txNum) {
		this.txNum = txNum;
	...
	public void loadTestbed() {
		...
		try {
			...
			if (WAREHOUSE_IDS == null)
				...
				for (int i = 0; i < WAREHOUSE_IDS.length; i++)
					generateWarehouseInstance(WAREHOUSE_IDS[i]);
		} catch (Exception e) {
			e.printStackTrace();
			tx.rollback();
			isCommitted = false;
